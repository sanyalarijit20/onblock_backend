...
* Updated for Ethers.js v6 BigInt compatibility.
*/
const airdropInitialFunds = async (userId, smartAccountAddress, network) => {
 try {
   ...
   // ETHERS V6 FIX: balance is a BigInt. Use standard mathematical operators.
   ...
   const wallet = await Wallet.findOne({ address: smartAccountAddress.toLowerCase() });
   ...
   tx.wait().then(async (receipt) => {
     if (dbTransaction.markConfirmed) {
